...
namespace PGSCTF.Services.Container.Provider;
...
public class KubernetesProvider : IContainerProvider<Kubernetes, KubernetesMetadata>
{
    ...
    private const string IsolatedNetworkPolicyName = "PGSCTF-network-isolated";
    private const string OpenNetworkPolicyName = "PGSCTF-network-open";
    ...
    private static readonly V1NetworkPolicy IsolatedNetworkPolicy = new()
    {
        ...
        Spec = new V1NetworkPolicySpec
        {
            PodSelector = new V1LabelSelector
            {
                ...
                {
                    ["PGSCTF.gzti.me/NetworkMode"] = nameof(NetworkMode.Isolated).ToLowerInvariant()
                }
            },
            PolicyTypes = ["Egress"],
            Egress = []
        ...
        new()
        {
            ...
            Spec = new()
            {
                PodSelector = new V1LabelSelector
                {
                    MatchLabels = new Dictionary<string, string>
                    {
                        ["PGSCTF.gzti.me/NetworkMode"] = nameof(NetworkMode.Open).ToLowerInvariant()
                    }
                },
                PolicyTypes = ["Egress"],
                Egress =
    ...
    {
        var padding = $"PGSCTF@{registry.UserName}@{address}".ToMD5String();
