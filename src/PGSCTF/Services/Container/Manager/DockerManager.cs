...
using ContainerStatus = PGSCTF.Utils.ContainerStatus;
...
public class DockerManager : IContainerManager
{
    ...
    public async Task<Models.Data.Container?> CreateContainerAsync(PGSCTF.Models.Internal.ContainerConfig config,
    ...
    private CreateContainerParameters GetCreateContainerParameters(PGSCTF.Models.Internal.ContainerConfig config) =>
        new()
        {
            Image = config.Image,
            Labels =
            ...
            Env = config.Flag is null
                ? [$"PGSCTF_TEAM_ID={config.TeamId}"]
                : [$"PGSCTF_FLAG={config.Flag}", $"PGSCTF_TEAM_ID={config.TeamId}"],
            HostConfig = new()
            {
