...
using System.Net;
using PGSCTF.Models.Internal;
using PGSCTF.Services.Container.Provider;
using k8s;
using k8s.Autorest;
using k8s.Models;

namespace PGSCTF.Services.Container.Manager;
...
public class KubernetesManager : IContainerManager
{
        ...
        CancellationToken token = default)
    {
        ...
        // PGSCTF_FLAG is Per-team dynamic flag issued & audited by the platform.
        //
        // Compliance & Abuse Notice:
        //
        // These env vars are integral to anti-abuse, audit trails and license compliance under
        // the Restricted License (LicenseRef-PGSCTF-Restricted). Unauthorized removal, renaming
        ...
        //
        // References: NOTICE, LICENSE_ADDENDUM.txt, licenses/LicenseRef-PGSCTF-Restricted.txt
        IList<V1EnvVar> envs = config.Flag is null
            ? [new V1EnvVar { Name = "PGSCTF_TEAM_ID", Value = config.TeamId }]
            :
            [
                new V1EnvVar { Name = "PGSCTF_FLAG", Value = config.Flag },
                new V1EnvVar { Name = "PGSCTF_TEAM_ID", Value = config.TeamId }
            ];
        ...
        var pod = new V1Pod
        {
            Metadata = new V1ObjectMeta
            {
                ...
                {
                    ["PGSCTF.gzti.me/ResourceId"] = name,
                    ["PGSCTF.gzti.me/Image"] = chalImage,
                    ["PGSCTF.gzti.me/TeamId"] = config.TeamId,
                    ["PGSCTF.gzti.me/UserId"] = config.UserId.ToString(),
                    ["PGSCTF.gzti.me/ChallengeId"] = config.ChallengeId.ToString(),
                    ["PGSCTF.gzti.me/NetworkMode"] = config.NetworkMode.ToString().ToLowerInvariant()
                }
            },
            Spec = new V1PodSpec
            {
        ...
        var service = new V1Service
        {
            ...
            Metadata = new V1ObjectMeta
            {
                ...
                Labels = new Dictionary<string, string> { ["PGSCTF.gzti.me/ResourceId"] = name }
            },
            Spec = new V1ServiceSpec
            {
                ...
                Selector = new Dictionary<string, string> { ["PGSCTF.gzti.me/ResourceId"] = name }
            }
        };
