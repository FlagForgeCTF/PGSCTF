...
namespace PGSCTF.Test.UnitTests.Models;
...
public class ChallengeFlagGenerationTests
{
    ...
    private static Participation CreateParticipation(Game game, int teamId = 1, string teamName = "Test Team")
    {
        ...
        var token = $"{team.Id}:{game.Sign($"PGSCTF_TEAM_{team.Id}", TestXorKey)}";
    ...
    [Theory]
    [InlineData(null, "flag{PGSCTF_dynamic_flag_test}")]
    [InlineData("", "flag{PGSCTF_dynamic_flag_test}")]
