using PGSCTF.Models;
using PGSCTF.Services.Container.Manager;
using PGSCTF.Services.Container.Provider;
using PGSCTF.Storage;
using PGSCTF.Storage.Interface;
...
public class PGSCTFApplicationFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
    ...
    public PGSCTFApplicationFactory()
    {
        // Check environment variable to determine test mode
        var testMode = Environment.GetEnvironmentVariable("PGSCTF_INTEGRATION_TEST_MODE")?.ToLowerInvariant() ?? "local";

        _useK3sMode = testMode == "cloud" || testMode == "k3s";
        _useMinioStorage = testMode == "cloud" || testMode == "minio";

        Console.WriteLine($@"[PGSCTFApplicationFactory] Test mode: {testMode}");

        // Initialize containers based on mode
        if (_useMinioStorage)
        {
            Console.WriteLine(@"[PGSCTFApplicationFactory] Creating MinIO container...");
        ...
        if (_useK3sMode)
        {
            Console.WriteLine(@"[PGSCTFApplicationFactory] Creating K3s container...");
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureAppConfiguration((_, config) =>
        {
            ...
            if (_useK3sMode && _k3sContainer is not null && !string.IsNullOrEmpty(_kubeConfigPath))
            {
                ...
                configDict["ContainerProvider:KubernetesConfig:Namespace"] = "PGSCTF-test";
    ...
    private async Task InitializeMinioAsync()
    {
        ...
        _storageConnectionString =
            $"minio.s3://bucket=PGSCTF-test;endpoint={minioEndpoint};accessKey=minioadmin;secretKey=minioadmin;forcePathStyle=true;useHttp=true;";
    }
    ...
    public async Task InitializeAsync()
    {
        ...
        Environment.SetEnvironmentVariable("PGSCTF_ConnectionStrings__Database", _connectionString);
...
public class IntegrationTestCollection : ICollectionFixture<PGSCTFApplicationFactory>;
